...
executable = 'distance-distribution-closed-syncmer.py'
kmer_length = 6
smer_length = 2
max_distance = 5
...
for is_test_probabilities in is_test_probabilities_runs:
    command = f'python {executable} -k {kmer_length} -s {smer_length} -m {max_distance} {is_test_probabilities}'
...
executable = 'distance-distribution-open-syncmer.py'
kmer_length = 6
smer_length = 2
t_offset_of_smer = 4
max_distance = 23
...
for is_test_probabilities in is_test_probabilities_runs:
    command = f'python {executable} -k {kmer_length} -s {smer_length} -t {t_offset_of_smer} -m {max_distance} {is_test_probabilities}'
    with open(log, "a") as fh:
        fh.write(command + "\n")
    command = f'{command} >> {log}'
    system( f'{command}' )
