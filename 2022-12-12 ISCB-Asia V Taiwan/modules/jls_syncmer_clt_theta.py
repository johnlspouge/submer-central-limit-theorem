...
# Returns the _standardized_variate_w W(theta, length_genome) 
#   in the central limit theorem about theta.
def _standardized_variate_w(
    ...
    _standardized_variate_w = count_of_unmutated_syncmers-theta_bar**k*count_of_syncmers
    if sigma_l == 0.0:
        if _standardized_variate_w < 0.0:
            return float('-inf')
...
def _test_standardized_variate_w():
    ...
    z = _standardized_variate_w(theta, syncmer, length_genome, count_of_syncmers, count_of_unmutated_syncmers)
...
def theta_all(count_of_syncmers, count_of_unmutated_syncmers, z_half_epsilon, syncmer, length_genome ):
    ...
    def _update(cross0, interval0, theta0, interval, theta):
        ...
        def _zero(theta):
            w = _standardized_variate_w(theta, syncmer, length_genome, count_of_syncmers, count_of_unmutated_syncmers)
            return w-value
    ...
    return theta_minus, _theta_zero(theta_minus, theta_plus), theta_plus
...
def _test_theta_all():
    ...
    theta_all0s = [
        [[[0.0, 0.0007734865796875001],
        [0.0007734865796875001, 0.0009015489609375002],
        [0.0009015489609375002, 1.0]],
        [[0.0, 0.0003947706015625],
        [0.0003947706015625, 0.0017629700265624998],
        [0.0017629700265624998, 1.0]],
        [[0.0, 1.3862220312500001e-05],
        [1.3862220312500001e-05, 0.0423956341611553],
        [0.0423956341611553, 1.0]]],
        
        [  # alpha = 0.05
            [  # length_genome = 1000000
                [0.0, 0.0083097677921875],
                [0.0083097677921875, 0.008719889891191962],
                [0.008719889891191962, 1.0]
            ],[ # length_genome = 10000
                [0.0, 0.0066882563265625],
                [0.0066882563265625, 0.010812503117187499],
                [0.010812503117187499, 1.0]
            ],[ # length_genome = 100
                [0.0, 0.0010934997265624996],
                [0.0010934997265624996, 0.05574933284552357],
                [0.05574933284552357, 1.0]
            ]
        ],
        
        [[[0.0, 0.1083230310248542],
        [0.1083230310248542, 0.10988257118642374],
        [0.10988257118642374, 1.0]],
        [[0.0, 0.10146205195350158],
        [0.10146205195350158, 0.11704388602761136],
        [0.11704388602761136, 1.0]],
        [[0.0, 0.0500725099480111],
        [0.0500725099480111, 0.1944894128421653],
        [0.1944894128421653, 1.0]]],

        [[[0.0, 0.3170349083992985],
        [0.3170349083992985, 0.3203777585756129],
        [0.3203777585756129, 1.0]],
        [[0.0, 0.30185928522966404],
        [0.30185928522966404, 0.3352391716966739],
        [0.3352391716966739, 1.0]],
        [[0.0, 0.16281319724520252],
        [0.16281319724520252, 0.4566894464114083],
        [0.4566894464114083, 1.0]]]    
    ]
    alpha = 0.05
    ...
    for i in range(len(length_genomes)):
        for j in range(len(fractions)):
            length_genome = length_genomes[i]
        ...
        isclose(theta_mean_, theta_mean0s[i])
        allclose(theta_all_,theta_all0s[j][i])
