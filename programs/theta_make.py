...
for submer0param in submer0params:
    ...
    for count in counts:
        ...
        command = f'python theta-from-{submer}-count.py {param} -n {count} -u {unmutated_count} -c 0.95'
...
command = f'python theta-from-{submer}-count.py {param} -n {count} -u {unmutated_count} -c 0.95 -l {length_estimate}'
with open(log, "a") as fh:
    fh.write(command + "\n")
command = f'{command} >> {log}'
system( f'{command}' )
